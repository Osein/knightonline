...
BOOL CN3UIString::GetTextExtent(const std::string& szString, int iStrLen, SIZE* pSize)
{
	if (m_pDFont) return m_pDFont->GetTextExtent(szString, iStrLen, pSize);
	return FALSE;
}

uint32_t CN3UIString::GetFontColor() const {
	if (m_pDFont)
		return m_pDFont->GetFontColor();
	
	return 0xffffffff;
}
const std::string& CN3UIString::GetFontName() const {
	if (m_pDFont)
...
uint32_t CN3UIString::GetFontHeight() const {
	if (m_pDFont)
...
uint32_t CN3UIString::GetFontFlags() const {
	if (m_pDFont)
