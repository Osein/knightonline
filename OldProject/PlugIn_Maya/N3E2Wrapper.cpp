...
CN3E2Wrapper::CN3E2Wrapper()
{
	...
	assert(m_hDlgProgress, "Failed to create progress dialog");
...
CN3Light* CN3E2Wrapper::ProcessLight(MFnLight &mLight)
{
	...
	auto mT = new MFnTransform(mLight.parent(0));
...
void CN3E2Wrapper::SceneExport()
{
	...
	HINSTANCE hInst = MhInstPlugin;
	HWND hWnd = M3dView::applicationShell();
	int rval = DialogBoxParamA(hInst, MAKEINTRESOURCE(IDD_EXPORT_OPTION), hWnd, DlgProcPane, NULL); // 컨트롤 패널 대화상자..
	if(rval != 1) return;
...
//				::SendMessage(m_hWndLB, LB_SETCURSEL, (WPARAM)nLI,0); // Progress dialog
	...
	if(bSaveOK)
	{
		...
		std::string fullPath(m_szPath);
		fullPath.append(m_szFileName);
		std::replace(fullPath.begin(), fullPath.end(), '/', '\\');

		m_pScene->SaveDataAndResourcesToFile(fullPath);
		m_pScene->Release();
	}
...
//		}
	...
	for(auto i = 0; i < nUVC; i++) pIMesh->UVSet(i, mFAU[i], 1.0f - mFAV[i]); // UV 데이터 세팅..
...
bool CN3E2Wrapper::ProcessSkin(MFnSkinCluster &mSkin, CN3Skin* pSkin)
{
	...
	assert(mMeshArray.length() >= 1, "Input geometry count is not 1");
...
//	}
	...
	for(auto i = nFN - 1; i >= 0; i--) // 저장할 이름을 만든다..
	{
...
int CN3E2Wrapper::ProcessTransform(MFnTransform &mTransform, CN3Transform *pTransform)
{
	...
	wsprintfA(szInfo, "fullPath: %s", pTransform->m_szName);
	::SendMessage(m_hWndLB, LB_ADDSTRING, 0, (LPARAM)szInfo);
...
//			MTransformationMatrix mTMJ(mMtxJT);
	...
	assert(1 == mMeshArray.length(), "binding mesh count is not 1");
	MFnMesh mMeshOrg(mMeshArray[0]); // Original Mesh

	mSkin.getOutputGeometry(mMeshArray); 
	assert(1 == mMeshArray.length(), "Output mesh count is not 1");
...
*/
	...
	else if(false == bCollisionMesh) // 충돌 체크 메시 아니면....정상적으로 진행..
	{
		...
		assert(nPart >= 0 && nPart < 64, "Part count is bigger than 64");
		assert(nLOD >= 0 && nLOD < MAX_CHR_LOD, "LOD Count is bigger than MAX_CHR_LOD");
...
INT_PTR CALLBACK CN3E2Wrapper::DlgProcProgress(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch( uMsg ) 
    {
        case WM_INITDIALOG:
...
INT_PTR CALLBACK CN3E2Wrapper::DlgProcPane(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch(uMsg)
	{
	case WM_INITDIALOG:
