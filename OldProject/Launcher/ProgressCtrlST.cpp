...
void CProgressCtrlST::OnPaint() 
{
	...
	CBitmap buttonBG;
	buttonBG.LoadBitmap(IDB_PROGRESS_BG);
	CDC dcMem;
	...
	dc.StretchBlt(0, 0, MulDiv(374, iDpi, 96), MulDiv(16, iDpi, 96), &dcMem, 0, 0, 374, 16, SRCCOPY);
	...
	// Stretch the progress track
	int dpiScaledWidth = MulDiv(374, iDpi, 96);
	int dpiScaledHeight = MulDiv(16, iDpi, 96);
	::SetStretchBltMode(hdcTemp, HALFTONE);
	::StretchBlt(hdcTemp, 0, 0, dpiScaledWidth, dpiScaledHeight, hdcMem, 0, 0, 374, 16, SRCCOPY);

	// Copy from temporary DC to screen (only the percentage rectangle)
	if (rcCtrl.IsRectEmpty() == FALSE)
		//::StretchBlt(dc.m_hDC, rcCtrl.left, rcCtrl.top, rcCtrl.Width(), rcCtrl.Height(), hdcTemp, 0, 0, 374, 16, SRCCOPY);
		::BitBlt(dc.m_hDC, rcCtrl.left, rcCtrl.top - 1, rcCtrl.Width(), rcCtrl.Height(), hdcTemp, 0, 0, SRCCOPY);
